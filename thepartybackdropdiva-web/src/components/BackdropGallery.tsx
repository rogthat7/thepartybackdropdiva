...
export const BackdropGallery: React.FC = () => {
    ...
    if (loading) return <div className="p-8 text-center text-gray-500">Loading luxury backdrops...</div>;

    return (
        <section className="p-8 max-w-7xl mx-auto">
            <div className="text-center mb-12">
                <h2 className="text-4xl font-light text-gray-900 tracking-tight">Curated Backdrop Collection</h2>
                <div className="w-24 h-1 bg-gold-500 mx-auto mt-4 rounded"></div>
            </div>
            
            <div className="columns-1 sm:columns-2 md:columns-3 gap-6 space-y-6">
                {backdrops.map(bg => (
                    <div key={bg.id} className="break-inside-avoid relative group overflow-hidden rounded-xl shadow-lg bg-white">
                        {/* Placeholder image logic since real images might not exist locally */}
                        <div className="bg-gray-200 aspect-[3/4] w-full flex items-center justify-center text-gray-400">
