...
const FadingCollectionCover: React.FC<{ collection: BackdropCollectionDto }> = ({ collection }) => {
    ...
    // If no images in collection, fallback to coverImageUrl
    if (images.length === 0) {
        return collection.coverImageUrl ? (
            <img src={collection.coverImageUrl} alt={collection.name} className="object-cover w-full h-full transition-transform duration-1000 group-hover:scale-110" />
        ) : (
    ...
    return (
        <div className="w-full h-full relative transition-transform duration-1000 group-hover:scale-110 bg-gray-900">
            {images.map((url, idx) => (
                <img 
                    key={idx}
                    src={url} 
                    alt={`${collection.name} view ${idx + 1}`} 
                    className={`absolute inset-0 object-cover w-full h-full transition-opacity duration-[2000ms] ${idx === currentIndex ? 'opacity-100' : 'opacity-0'}`} 
                />
            ))}
        </div>
    );
...
export const BackdropGallery: React.FC<{ isDark?: boolean }> = ({ isDark = true }) => {
        ...
        <section className="p-8 max-w-7xl mx-auto min-h-screen">
                ...
                {collections.map(col => (
                    <div key={col.id} className="group relative bg-white dark:bg-gray-800 rounded-[2rem] overflow-hidden shadow-2xl transition-all duration-500 hover:-translate-y-2 border border-gray-100 dark:border-gray-700">
                        ...
                        <div className="aspect-[4/5] overflow-hidden relative">
                            <FadingCollectionCover collection={col} />
            ...
            {selectedCollection && createPortal(
                <div className={isDark ? 'dark' : ''}>
                    ...
                    >
                        <div className="max-w-[1800px] mx-auto min-h-screen px-8 md:px-16 pt-48 pb-32">
                            ...
                            >
                                ...
                                <div className="w-full md:w-[25%] flex flex-col items-start text-left animate-in slide-in-from-left duration-1000 sticky top-48">
